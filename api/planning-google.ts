...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const calendarRangeDays = clamp(parseIntegerParam(req.query.rangeDays, 30), 1, 180);
  const pastDays = clamp(parseIntegerParam(req.query.pastDays, 3), 0, 90);

  const explicitMin = parseDateParam(req.query.timeMin);
  const explicitMax = parseDateParam(req.query.timeMax);
